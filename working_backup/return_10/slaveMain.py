	...
	while slave.alive:
		...
		try:
			...
			if masterMessage['msgType'] == 'request':
				...
				requestList.addGlobalRequest(msgParser.parseString(masterMessage))

			elif masterMessage['msgType'] == 'elev_id':
				if slave.getSlaveID() != int(masterMessage['content']):
					slave.setSlaveID(msgParser.parseString(masterMessage))
		...
		elevPanel.updateLightsByRequestList(requestList.list + globalRequestList.list)
