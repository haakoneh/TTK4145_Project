...
from ElevNetworkCommunication import *	#change this filename for the love of god
import time
...
class Request_List:
	def __init__(self, elevator):
		...
		self.interntalFile = 'requestListFile.txt'
		self.externalRequestHandler = ExternalFileHandler('globalRequestFile.txt')
		self.readRequestFile()

	def addRequest(self):
		"""Adds request if it does not alredy exist in list"""
		self.externalRequestHandler.updateFile()
		externalRequest = self.externalRequestHandler.getFileListContent()
		if externalRequest:
			for request in externalRequest:
				request.pop()
				if request not in self.list and request != []:
			...
			for buttonType in range(3):
				if self.elevator.getButtonSignal(buttonType, floor) == 1:
					...
					if buttonType != INPUT.BUTTON_IN:
						request.append(0)
						self.externalRequestHandler.addFileContent(request)

					elif request not in self.list:
						self.list.append(request)
						self.printRequestList()

		self.externalRequestHandler.printFileContent()
		self.updateInternalRequestFile()

	def readRequestFile(self):
		"""Reads from file during initialization, only copies orders from inside elevator"""
		with open(self.interntalFile, 'r') as f:
	...
	def updateInternalRequestFile(self):
		"""Updates file whenever the request list is changed"""
		with open(self.interntalFile, 'w') as f:
		    for s in self.list:
		        f.write(str(s) + '\n')	
